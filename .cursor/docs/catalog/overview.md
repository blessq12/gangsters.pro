...
| **Товар** | Продаваемая позиция с ценой, описанием, пищевой ценностью, составом, тегами |
...
| **Meta-поля товара** | `meta_counts_as_roll`, `meta_gift_candidate`, `meta_is_complement_set` — только в БД/Filament, для Checkout ACL |

## Границы

| Внутри BC | Снаружи |
|-----------|---------|
| Структура меню, карточки товаров/наборов, теги, медиа товара, порядок в категории | Корзина, checkout, оплата, доставка, клиенты |
| Публичный read API `GET /api/catalog` | Checkout читает цены и meta через ACL-порты |
...
| Checkout ACL | `app/Infrastructure/Checkout/Port/Catalog*Adapter.php` |
...
- Checkout ACL: 4 порта (`CatalogPricingPort`, `CatalogGiftCandidatesPort`, `CatalogRollMetaPort`, `CatalogComplementSetCandidatesPort`).
