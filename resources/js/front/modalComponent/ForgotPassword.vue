...
export default {
    ...
    methods: {
        send() {
            this.schema.validate({ email: this.email }, { abortEarly: false })
                .then(() => {
                    this.errors = []
                    this.isLoading = true
        ...
        updateInputs() {
            let inputs = this.$refs.forgotPasswordForm.querySelectorAll('input')
            ...
            <form @submit.prevent="send" ref="forgotPasswordForm">
               <div class="input-group mb-3">
                ...
                <div v-if="isLoading" class="d-flex mb-3 align-items-center" >
                    ...
                    <span class="ms-2" v-if="!finish">Поиск учетной записи ...</span>
                </div>
                <span class="d-block mb-2 " v-if="finish">
                    <span class="text-success" v-if="issetEmail">{{ message }}</span>
                    <span class="text-danger" v-else>{{ message }}</span>
                </span>
                <button class="btn w-50 rounded" :disabled="isLoading">Отправить</button>
            </form>
        </div>
    </div>
</template>
