...
export default {
    ...
    data() {
        return {
            ...
            schema: object({
                ...
                dob: string().required().matches(/^\d{2}-\d{2}-\d{4}$/, 'Дата должна быть в формате ДД-ММ-ГГГГ').test('is-today-or-past', 'Дата не может быть больше сегодняшнего дня', value => {
                    if (!value) return true; 
                    const dateParts = value.split('-');
                    const date = new Date(`${dateParts[2]}-${dateParts[1]}-${dateParts[0]}`);
                    const today = new Date();
                    return date <= today && today.getFullYear() - date.getFullYear() >= 16; 
                }),
                tel: string().required(),
            }),
            errors: []
    ...
    methods: {
        ...
        update(){
            ...
            .then( res => {
                this.errors = []
                axios.patch('/api/auth/update-user', this.formData)
                .then( res => {
                    this.editMode = false
            ...
            <div class="col-12">
                <div class="card" v-if="activeTab == 'personal_data'">
                    <div class="card-body" v-if="!editMode">
                        ...
                        <p class="card-text"><b>Дата рождения:</b> {{ userStore.userData.dob ? userStore.userData.dob : 'Не указано' }}</p>
                    ...
                    <div class="card-body" v-else>
                        ...
                        <form @submit.prevent="update" ref="form">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="dob" v-maska  data-maska="##-##-####" v-model="formData.dob" placeholder="01-01-1990">
                            </div>
                            <div class="mb-3">
                                <label for="tel" class="form-label">Телефон</label>
                                <input type="tel" class="form-control" id="tel" v-maska data-maska="+7 (###) ###-##-##" v-model="formData.tel">
                            </div>
                            <div class="mb-3">
                                <button class="btn-exit fw-light" type="submit">Сохранить</button>        
                                <button class="btn-exit fw-light ms-2 " type="button" @click="editMode = false">Отменить</button>        
                            </div>
                        </form>
                ...
                <div class="card" v-if="activeTab == 'gcoins'">
                    <div class="card-body">
                        ...
                        <p class="card-text">У вас <b>{{ coins }}</b> G-Coins</p>
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body" style="background: #f5f5f5;">
                        ...
                        <p class="card-text">
                            Это необходимо для корректной работы системы кешбека. 
                            Чтобы мы могли отправлять акции и скидки именно тебе.
                        </p>
                    </div>
                </div>
            </div>
